...
rem Allow test cases to run in a thread
start "ldra" /wait /min %TBED%\TBini TBRUN_ENABLE_CREATE_TC_THROUGH_THREAD=TRUE
rem Allow infinite loops to be terminated via stub count
start "ldra" /wait /min %TBED%\TBini TBRUN_LOCAL_STUB_HIT_COUNTS=TRUE
start "ldra" /wait /min %TBED%\TBini TBRUN_TC_SETJMP=TRUE


rem Run the Main Static, Complexity Analysis, Data Flow, Information Flow & Instrumentation
rem =======================================================================================
start "ldra" /wait /min %TBED%\%TOOL% %SRC_FILES% /112a34021q

rem Run each sequence in the TCF directory
rem ======================================
set TCF_ROOT=%PRJ_ROOT%\LLR_Tests
for %%i in (%TCF_ROOT%\*.tcf) do (
  start "ldra" /wait /min %TBED%\%TOOL% %SRC_FILES% /1q /tbruntcf=%%i /tbruntcfargs="-regress -quit"
)
